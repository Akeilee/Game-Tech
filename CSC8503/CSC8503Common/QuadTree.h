		...
		protected:
			...
			void Insert(T& object, const Vector3& objectPos, const Vector3& objectSize, int depthLeft, int maxSize) {
				if (!CollisionDetection::AABBTest(objectPos, Vector3(position.x, 0, position.y), objectSize, Vector3(size.x, 1000.0f, size.y))) {
					return;  // not overlapping itself, thus no overlapping children
				}
				//if does overlap
				if (children) { //if not a leaf node , just descend the tree - add object to children node
					for (int i = 0; i < 4; ++i) {
						children[i].Insert(object, objectPos, objectSize, depthLeft - 1, maxSize);
					}
				}
				else { //currently a leaf node , can just expand and add object to own list
					contents.push_back(QuadTreeEntry <T>(object, objectPos, objectSize));

					if ((int)contents.size() > maxSize && depthLeft > 0) { //if not reached max depth of tree or not reached max number of objects per node
						if (!children) {
							Split(); //if reached max objects per node, split
							//we need to reinsert the contents so far into child list!
							...
							contents.clear(); // contents now distributed - in child nodes now so can clear!
			...
			void Split() { //splitting area into 4 children nodes
			...
			void OperateOnContents(QuadTreeFunc& func) { //operate on contents of quadtree using typedef function
				if (children) {
					for (int i = 0; i < 4; ++i) {
					...
					if (!contents.empty()) { //if reached leaf node/no more children nodes
						func(contents);
					}
				}
			}
